...
At tree level, the MSSM  Higgs bosons masses, decay branching fraction and production cross section are all determined by two parameter,
by convention chosen to be $M_A$ and $\tan\beta$. As it has been pointed out in Section~\ref{sec:hsector}, radiative corrections
contribute significantly to the MSSM Higgs bosons masses and the prediction of physics observables becomes dependent on several parameters.
...
important. Furthermore, these corrections are dependent on the SUSY-breacking scale $M_{SUSY}$, the trilinear Higgs-stop Higgs-sbottom Yukawa 
couplings, the electroweak gaugino and gluino mass parameters.
...
studies. To cope with this difficulty several benchmark scenarios has been proposed~\cite{LHCxsec}, where the SUSY parameters, entering via radiative corrections, 
...
For the past few decades, the $m_h^{max}$ scenario~\cite{MSSMmhmax} was one of the most interesting benchmark scenarios, here
the parameter that contributes to radiative corrections are fixed such that the mass of the light CP-even Higgs boson, 
$M_h$, is maximal under the variation of $M_A$ and $\tan\beta$. This benchmark
scenario allows to set conservative lower bounds on $M_A$, $M_H^{\pm}$ and $\tan\beta$~\cite{Xsec243}. The $m_h^{max}$  scenario was used in the past 
to set limits at LEP, Tevatron and LHC~\cite{LEPLimits,TevatronLimits1,CMSLimit,ATLASLimit}, 
however, given the recent discovery of a Higgs boson with mass $\sim 126$ GeV, this scenario
